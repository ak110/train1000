...
def _main():
    ...
    parser.add_argument(
        "--model", default="default", choices=("default", "resnet25", "vgg10")
    )
    ...
    if hvd.rank() == 0:
        ...
        handlers.append(
            logging.FileHandler(
                args.results_dir / f"{args.data}.{args.model}.log", encoding="utf-8"
            )
        )
    logging.basicConfig(
    ...
    keras.utils.plot_model(
        model, args.results_dir / f"{args.data}.{args.model}.svg", show_shapes=True
    )
    ...
    if hvd.rank() == 0:
        ...
        logger.info(
            f"Test Accuracy:      {sklearn.metrics.accuracy_score(y_test, pred_test.argmax(axis=-1)):.4f}"
        )
        logger.info(
            f"Test Cross Entropy: {sklearn.metrics.log_loss(y_test, pred_test):.4f}"
        )
        # 後で何かしたくなった時のために一応保存
        model.save(
            args.results_dir / f"{args.data}.{args.model}.h5", include_optimizer=False
        )
    ...
    def call(self, inputs, training=None):  # pylint: disable=arguments-differ
        ...
        def _mixfeat():
            ...
            def _forward(x):
                ...
                rs = keras.backend.concatenate(
                    [keras.backend.constant([1], dtype="int32"), shape[1:]]
                )
                r = keras.backend.random_normal(rs, 0, self.sigma, dtype="float32")
                theta = keras.backend.random_uniform(
                    rs, -np.pi, +np.pi, dtype="float32"
                )
...
def _generate(X, y, batch_size, num_classes, shuffle=False, data_augmentation=False):
    ...
    if data_augmentation:
        aug1 = A.Compose(
            [
                A.PadIfNeeded(
                    40, 40, border_mode=cv2.BORDER_CONSTANT, value=[127, 127, 127], p=1
                ),
                _create_autoaugment(),
...
class Color(A.ImageOnlyTransform):
    ...
    def __init__(self, mag=10, always_apply=False, p=0.5):
        super().__init__(always_apply, p)
        self.mag = mag
